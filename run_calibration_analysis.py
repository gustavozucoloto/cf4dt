...
posterior_log = np.load("data/posterior_logarithmic.npy")
...
print(f"Posterior logarithmic shape: {posterior_log.shape}")
...
bundle_log = joblib.load("data/gp_logarithmic.joblib")

# Regenerate training data from the artificial data
X_train_pl, y_train_pl = build_training_set(
    df_truth,
    model_name="powerlaw",
    n_theta=20,
    seed=1,
    use_subset_points=32,
    n_jobs=args.n_jobs,
)
X_train_exp, y_train_exp = build_training_set(
    df_truth,
...
X_train_log, y_train_log = build_training_set(
    df_truth,
...
ax.scatter(np.arange(len(y_train_pl)), y_train_pl[sort_idx_pl], alpha=0.6, s=30, 
           label="Artificial data", color="C1")
ax.plot(np.arange(len(y_train_pl)), mu_pl[sort_idx_pl], 'C0-', lw=2, label="GP mean")
ax.fill_between(np.arange(len(y_train_pl)), 
...
ax.scatter(np.arange(len(y_train_exp)), y_train_exp[sort_idx_exp], alpha=0.6, s=30, 
           label="Artificial data", color="C1")
ax.plot(np.arange(len(y_train_exp)), mu_exp[sort_idx_exp], 'C2-', lw=2, label="GP mean")
ax.fill_between(np.arange(len(y_train_exp)), 
...
# Logarithmic
ax = axes[2]
sort_idx_log = np.argsort(mu_log)
ax.scatter(np.arange(len(y_train_log)), y_train_log[sort_idx_log], alpha=0.6, s=30, 
           label="Artificial data", color="C1")
ax.plot(np.arange(len(y_train_log)), mu_log[sort_idx_log], 'C3-', lw=2, label="GP mean")
ax.fill_between(np.arange(len(y_train_log)), 
                 (mu_log - 2*std_log)[sort_idx_log], 
                 (mu_log + 2*std_log)[sort_idx_log], 
                 alpha=0.3, color='C3', label="95% PI")
ax.set_xlabel("Sample index (sorted)")
ax.set_ylabel("$Q_{lc}$ (kW)")
ax.set_title("Logarithmic: GP vs. Data")
ax.legend()
ax.grid(True, alpha=0.3)
...
rmse_pl = np.sqrt(np.mean((y_train_pl - mu_pl)**2))
mae_pl = np.mean(np.abs(y_train_pl - mu_pl))
r2_pl = 1 - np.sum((y_train_pl - mu_pl)**2) / np.sum((y_train_pl - y_train_pl.mean())**2)

rmse_exp = np.sqrt(np.mean((y_train_exp - mu_exp)**2))
mae_exp = np.mean(np.abs(y_train_exp - mu_exp))
r2_exp = 1 - np.sum((y_train_exp - mu_exp)**2) / np.sum((y_train_exp - y_train_exp.mean())**2)

rmse_log = np.sqrt(np.mean((y_train_log - mu_log)**2))
mae_log = np.mean(np.abs(y_train_log - mu_log))
r2_log = 1 - np.sum((y_train_log - mu_log)**2) / np.sum((y_train_log - y_train_log.mean())**2)

coverage_pl = np.mean((y_train_pl >= mu_pl - 2*std_pl) & (y_train_pl <= mu_pl + 2*std_pl))
coverage_exp = np.mean((y_train_exp >= mu_exp - 2*std_exp) & (y_train_exp <= mu_exp + 2*std_exp))
coverage_log = np.mean((y_train_log >= mu_log - 2*std_log) & (y_train_log <= mu_log + 2*std_log))
...
print(f"\nLogarithmic:")
print(f"    RMSE: {rmse_log:.4f} kW")
print(f"    R²:   {r2_log:.6f}")
print(f"    Coverage 95%: {coverage_log:.1%}")
...
theta_median_log = np.median(posterior_log, axis=0)
...
alpha_log_vals = alpha_model(T_range, theta_median_log, model='logarithmic')
...
ax.plot(T_range, alpha_log_vals, 'C3:', lw=2, label="Logarithmic (calibrated)")
...
W_mph_range = np.linspace(0.05, 5.0, n_scenarios)
...
idx_log = np.random.choice(len(posterior_log), size=min(n_samples_uq, len(posterior_log)), replace=False)

samples_pl = posterior_pl[idx_pl]
samples_exp = posterior_exp[idx_exp]
samples_log = posterior_log[idx_log]

Q_med_pl = np.zeros((len(Ts_range), len(W_mph_range)))
Q_lower_pl = np.zeros_like(Q_med_pl)
Q_upper_pl = np.zeros_like(Q_med_pl)

Q_med_exp = np.zeros((len(Ts_range), len(W_mph_range)))
Q_lower_exp = np.zeros_like(Q_med_exp)
Q_upper_exp = np.zeros_like(Q_med_exp)

Q_med_log = np.zeros((len(Ts_range), len(W_mph_range)))
Q_lower_log = np.zeros_like(Q_med_log)
Q_upper_log = np.zeros_like(Q_med_log)

for i_ts, Ts in enumerate(Ts_range):
    for i_w, W_mph in enumerate(W_mph_range):
        # Powerlaw
        X_pl = np.column_stack([
            np.full(len(samples_pl), W_mph),
        ...
        X_log = np.column_stack([
            np.full(len(samples_log), W_mph),
            np.full(len(samples_log), Ts),
            samples_log[:, 0],
            samples_log[:, 1]
        ])
        mu_log_uq, std_log_uq = gp_predict(bundle_log, X_log)
        Q_med_log[i_ts, i_w] = np.median(mu_log_uq)
        Q_lower_log[i_ts, i_w] = np.percentile(mu_log_uq, 2.5)
        Q_upper_log[i_ts, i_w] = np.percentile(mu_log_uq, 97.5)

print(f"Generated predictions for {len(Ts_range)}×{len(W_mph_range)} scenarios")
...
ci_width_log = Q_upper_log - Q_lower_log
...
for Ts_pick in Ts_select:
    i_ts = np.argmin(np.abs(Ts_range - Ts_pick))
    ax.fill_between(W_mph, Q_lower_log[i_ts], Q_upper_log[i_ts], alpha=0.2)
    ax.plot(W_mph, Q_med_log[i_ts], marker='o', label=f"Ts={Ts_range[i_ts]:.0f} K", markersize=4)
ax.set_xscale('linear')
ax.set_xlabel("W (m/h)")
ax.set_ylabel("$Q_{lc}$ (kW)")
ax.set_title("Logarithmic: Posterior Predictive Q_lc Slices")
ax.legend(fontsize=9)
ax.grid(True, alpha=0.3)
...
ax.hist(ci_width_log.flatten(), bins=30, alpha=0.6, label="Logarithmic", density=True)
...
ratio_log = ci_width_log / (2 * (np.abs(Q_med_log) + 1e-10))
...
ax.scatter(Q_med_log.flatten(), ratio_log.flatten(), alpha=0.5, s=20, label="Logarithmic")
...
cv_log = np.median(ci_width_log / (2 * Q_med_log + 1e-10))
...
EMULATOR QUALITY:
  ...
  Mean Absolute Error vs. Ulamec:
    ...
    • Exponential:      {np.mean(np.abs(alpha_exp_vals - alpha_ulamec_vals)):.6f}
        • Logarithmic:      {np.mean(np.abs(alpha_log_vals - alpha_ulamec_vals)):.6f}
